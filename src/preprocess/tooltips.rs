        ...
        match self.tooltip_type {
            ...
            TooltipType::Inline(title, content) => {
                let tooltip_html = format!(
                    r#"<div class="text-tooltipped">{}<span class="text-tooltip-content">{}</span></div>"#,
                    title, content
                );
...
mod tests {
    ...
    fn test_replace_all_escaped() {
        ...
        let expected = r#"<div class="text-tooltipped">random<span class="text-tooltip-content">not determined</span></div>"#;
